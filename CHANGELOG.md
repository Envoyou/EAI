...
- **Canonical Review Capability and Candidate Draft Availability**:
  ...
  - Recovered deterministic paragraph-boundary patches when a model returns descriptive `targetText` but names one unique exact article anchor in its instruction; ambiguous or already-separated anchors remain manual.
  - Routed provider SEO findings to their canonical `publication.*` fields, discarded publication-metadata leakage from content-only Fast checks, and let Refine regenerate publication-only failures before exposing a human decision.
  - Replaced body-editor fallback for unresolved publication metadata with a direct SEO Pack field handoff; manual metadata saves now trigger automatic publication validation while export remains guarded.
...
- **Separated editorial workspaces**:
  ...
  - Extended that lineage back to the article origin: a committed Strategist Blueprint, Quick Draft, or Draft from Notes now issues the durable `sourceRef` carried into Editor, while each later Analysis artifact is linked to the tenant-scoped root artifact. Chat sessions may contain multiple explorations and therefore do not become article families until an output is committed.
  - Completed the Editor-to-workflow handoff: the source draft remains locked and visible while a dedicated pipeline panel runs, then the durably saved result offers Review or Publication according to its current unresolved findings. Finish later returns to the document home without losing the result.
  - Carried the generated SEO package with a ready article into Publication and moved its title, slug, excerpt, meta fields, cover alt text, tags, and metadata decisions into a dedicated right panel instead of stacking them above the article. Mobile Publication exposes the same separation through Article and SEO Pack tabs.
- **Decision-oriented pre-final review**:
  ...
  - Made publication-field application atomic: the backend now validates the current revision and server-stored finding, then persists the selected metadata value and `isApplied` resolution in one serializable transaction. The general metadata-save readiness guard remains unchanged.
  - Made the final article body's dominant language authoritative for SEO metadata and Quality Gate prose. A mismatched SEO result receives one internal corrective retry before any package or feedback is emitted, so language drift caused by rewrite/profile defaults is remediated without an editor decision.
...
- **In-place Final Draft editing**:
  ...
  - Added backend-issued persistent identities for Quality Gate findings, validator rules, factual/source-sensitive claims, target blocks, and canonical research/verified sources. History and rerun reconciliation now prefer these identities while retaining legacy text matching only for older saved records; browser-supplied identity fields are discarded and restored solely from server-persisted findings before an editor decision can enter the resolution ledger.
  - Added dependency-aware `ValidationScope` for automatic post-mutation checks. Formatting-only revisions use deterministic checks without an AI call, bounded editorial changes use lightweight validation, and bounded factual/source-sensitive changes validate only changed blocks, neighboring context, linked claims/sources, and global intro/conclusion anchors. Missing lineage, topic shifts, and broad multi-block changes still escalate to the full Quality Gate. Results remain revision/hash guarded and are reused idempotently for the same exact revision.
  - Added non-blocking background validation after a manual Final Draft revision is durably saved. Safe formatting/copy edits schedule no work; lightweight revisions debounce for 1.8 seconds and high-risk revisions for 800 ms. New saves and conflicting workspace actions cancel/coalesce pending work, editors can continue typing while checks run, and the UI reports completion or remaining decisions without instructing the user to rerun Quality Gate.
  - Replaced package-wide SEO invalidation with persistent per-field dependency state. Safe fields (`excerpt`, meta description, cover alt text, and tags) refresh in the background only when their prior content depends on the changed draft; title, meta title, and slug are never overwritten automatically and remain explicit editorial decisions. Any unresolved field still keeps the publication package stale and export blocked.
  - Hid validation and SEO pipeline mechanics from the normal editorial UX. Final Draft now derives one outcome-oriented state—checking, completed, content decision needed, or publication-detail decision needed—without exposing stale/rerun terminology. Manual revision checks and publication-detail refreshes remain conditional recovery tools, while Apply actions describe the editorial change and treat verification as system responsibility.
  - Made remediation convergence explicit: applying a suggested edit is provisional and can no longer suppress the same unresolved finding on the next validation. Only an editor-accepted warning or a verified source enters the durable resolution ledger; legacy applied records and pre-fix cached validation results are ignored.

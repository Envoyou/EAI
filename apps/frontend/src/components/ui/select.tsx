...
const selectTriggerVariants = cva(
  "flex w-full items-center justify-between gap-1.5 rounded-lg border border-[var(--border)] bg-[var(--surface-3)] py-2 pr-2 pl-2.5 text-sm whitespace-nowrap transition-colors outline-none select-none focus-visible:border-[var(--primary)] focus-visible:ring-2 focus-visible:ring-[var(--primary)]/35 focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--background)] disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 data-placeholder:text-muted-foreground data-[size=default]:h-8 data-[size=sm]:h-7 data-[size=sm]:rounded-[min(var(--radius-md),10px)] *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-1.5 dark:bg-[var(--surface-3)] dark:border-[var(--border)] dark:hover:bg-[var(--surface-3)]/80 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        default: null,
        surface: "ui-control ui-select",
      },
    },
    defaultVariants: {
      variant: "default",
...
function Select<Value, Multiple extends boolean | undefined = false>({
  ...
  React.useEffect(() => {
    ...
    const closeOnAncestorScroll = (event: Event) => {
    ...
    window.addEventListener("scroll", closeOnAncestorScroll, true)
    return () => window.removeEventListener("scroll", closeOnAncestorScroll, true)
  }, [controlledOpen, open])

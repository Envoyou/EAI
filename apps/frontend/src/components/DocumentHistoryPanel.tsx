...
import { Plus, FileText, Loader2, Search, Trash2, X, ChevronLeft } from 'lucide-react';
    ...
    <div className="flex flex-col h-full bg-[var(--surface-1)]">
      ...
      <div className="flex items-center justify-between px-3 py-3 border-b border-[var(--border)]">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-xs font-semibold text-[var(--foreground)] truncate">Documents</span>
        </div>
        {onToggle && (
          <button
            onClick={onToggle}
            className="p-1 rounded-md hover:bg-[var(--surface-2)] text-[var(--muted-foreground)] shrink-0"
      ...
      <div className="flex gap-1 px-2 pt-2">
        {FILTERS.map(f => (
          <button
            ...
            className={`px-2 py-1 text-[10px] font-medium rounded-full transition-colors border-none cursor-pointer ${
              activeFilter === f.key
      ...
      {/* Document List */}
      <ScrollArea className="flex-1 min-h-0 px-2 pt-2 pb-2">
        {loading ? (

...
- **Publication finding routing**: A `targetField` under `publication.*` may mutate only its mapped `PublicationPackage` field. It must never enter the body Targeted Fix fallback. Deterministic metadata proposals such as bounded slug shortening are prepared before rendering; Use/Keep/Edit decisions must not invoke an AI model. Persist the selected metadata value and its finding resolution through the atomic `apply_publication_metadata_finding` history action; never split them into independent resolution and metadata-save requests.
- **Source-fidelity remediation**: Do not offer an open-ended generative Rewrite action for source-fidelity or verification findings. Offer the narrow source action, deterministic remove/neutralize when target text exists, or a genuine editor decision; the resulting saved body is always revalidated by the system. A bounded `needs_citation` warning may expose Keep current text only when it has an exact visible target and no prepared body patch. Never expose that acceptance path for blocking failures or `high_risk_factual_claim` findings.
- **Residual decision actions**: Refine normally returns only the final result. If a bounded backend remediation cannot safely finish a finding, present one approval-oriented action that applies the supplied correction and automatically validates it; do not make the editor separately accept a finding and then click Rewrite. Pure acceptance remains available only when no body change is being proposed.
- **Prepared-patch acceptance**: An action labeled Accept change must already have exact target text, replacement text, and operation. Apply that patch optimistically before persistence, roll back only when persistence fails, and reload the authoritative saved revision on a revision conflict. A remaining validation warning is not a persistence failure and must keep the accepted edit. Findings without a prepared patch expose Generate suggestion first; generation only creates a Before/After preview and must not mutate or persist the draft until the editor accepts it.
- **Pre-final review boundary**: A persisted candidate with unresolved blocking editorial decisions must render as Draft Review, not Final Draft. Keep the candidate body hidden by default, expose manual editing only as an explicitly labeled Candidate Draft view, and promote the normal Final Draft surface only after the saved revision reaches `ready`.
- **Editorial workspace boundaries**: `/workspace` is the document home/library, `/editor` owns writing and refinement, `/review` owns editorial findings and decisions, and `/publication` owns metadata plus export. Do not restore the saved-article list or the all-purpose Chat/Feedback/Notes/Report tab set as permanent columns around the editor. Editor tools are opt-in, Review exposes only evaluation tools, and Publication must reject unresolved review state. Legacy `/workspace?...` document links redirect to `/editor?...` so external navigation remains compatible.
- **Active queue projection**: Workspace, Review, and Publication cards represent the current article family keyed by durable `sourceRef`, never individual AnalysisLog revisions. Review requires a current review/blocked snapshot with unresolved findings; Publication requires current ready state with zero unresolved findings. Preserve `sourceRef` through Analyze, Refine, reload, and manual-save flows. Historical findings belong in revision/audit surfaces, not active queues.
- **Saved-library destructive actions**: Selection and Select All must operate only on the currently visible search/filter result set and reset when that view changes. Draft cards open Editor, review/blocked cards open Review, and ready cards open Publication. Family deletion must disclose that all saved revisions are removed, remain pending-guarded, and rely on the backend to synchronize Content Memory.
- **Article-family origin**: A Strategist chat session is an exploration container, not an article. Start one article family when the user commits a Blueprint, Quick Draft, or Draft from Notes; carry the backend-issued `sourceRef` into Editor metadata and preserve an existing `sourceRef` when Strategist tools are used inside an already-open article.
- **Editor workflow handoff**: While initial Refine runs, keep the source draft visible and locked and replace the optional tool panel with the pipeline status; do not expose partial candidate controls. After the backend has durably completed the record, route unresolved current findings to Review and a ready result to Publication through an explicit handoff. A generated current SEO package travels with that record and must be readable and editable in Publication's dedicated right panel without returning to Editor or stacking metadata above the article; use a separate SEO Pack tab on narrow screens.

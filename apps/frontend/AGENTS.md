...
- **Source-fidelity remediation**: Do not offer an open-ended generative Rewrite action for source-fidelity or verification findings. Offer the narrow source action, deterministic remove/neutralize when target text exists, or a genuine editor decision; the resulting saved body is always revalidated by the system.

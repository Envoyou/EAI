  ...
  try {
    ...
    if (history && Array.isArray(history)) {
      ...
      chatHistory = truncationNote + trimmed
        .map((m: { role: string; content: string; payload?: { sources?: { url: string; domain: string }[] } }) => {
    ...
    // Helper to align a domain-only or short URL string with the full verified URL registry
    const resolveToFullVerifiedUrl = (sourceInput: string, registry: string[]): string => {
      if (!sourceInput || typeof sourceInput !== 'string') return sourceInput;
      
      const cleanInput = sourceInput.trim().replace(/^https?:\/\/(www\.)?/, '').toLowerCase();
      
      // 1. Check exact match
      const exactMatch = registry.find(u => u.toLowerCase() === sourceInput.toLowerCase());
      if (exactMatch) return exactMatch;

      // 2. Check domain hostname match
      for (const verifiedUrl of registry) {
        try {
          const verifiedUrlObj = new URL(verifiedUrl);
          const verifiedHost = verifiedUrlObj.hostname.replace('www.', '').toLowerCase();
          
          if (cleanInput === verifiedHost || cleanInput === verifiedHost + verifiedUrlObj.pathname.toLowerCase()) {
            return verifiedUrl;
          }
          
          if (verifiedHost.includes(cleanInput) || cleanInput.includes(verifiedHost)) {
            return verifiedUrl;
          }
        } catch {
    ...
    // Align all blueprint sources generated by the model
    if (data.plan && Array.isArray(data.plan.sources)) {
      const alignedSources = data.plan.sources.map(src => resolveToFullVerifiedUrl(src, validUrlsRegistry));
      data.plan.sources = [...new Set(alignedSources)];
    }

    // Now update cite placeholders in draft using resolved sources list
    if (data.plan && Array.isArray(data.plan.sources) && data.plan.sources.length > 0) {
      const resolvedSources = data.plan.sources;
      ...
      const replaceCiteFallback = (text: string) => {
        ...
        return text.replace(citeRegex, (match, citeVal) => {
          ...
          // Fuzzy search index matching if the model used source domain names in cite
          const cleanCite = citeVal.trim().toLowerCase();
          const foundIdx = resolvedSources.findIndex(url => {
            try {
              return new URL(url).hostname.replace('www.', '').toLowerCase().includes(cleanCite);
            } catch {
              return false;
            }
          });

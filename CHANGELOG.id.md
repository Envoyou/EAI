...
- **Capability Review Kanonis dan Ketersediaan Draft Kandidat**:
  ...
  - Memulihkan patch batas paragraf deterministik ketika model mengirim `targetText` deskriptif tetapi menyebut satu anchor artikel yang unik dan persis di instruksinya; anchor ambigu atau yang sudah terpisah tetap manual.
  - Mengarahkan finding SEO dari provider ke field `publication.*` kanonis, membuang kebocoran metadata publikasi dari pemeriksaan Fast yang hanya mengaudit konten, dan membuat Refine meregenerasi kegagalan khusus publikasi sebelum meminta keputusan manusia.
  - Mengganti fallback editor body untuk metadata publikasi unresolved dengan handoff langsung ke field SEO Pack; penyimpanan metadata manual kini memicu validasi publikasi otomatis sementara guard ekspor tetap aktif.
...
- **Pemisahan Ruang Kerja Editorial**:
  ...
  - Memperluas lineage tersebut hingga asal artikel: Strategist Blueprint, Quick Draft, atau Draft from Notes yang sudah dipilih kini menerbitkan `sourceRef` durable yang dibawa ke Editor, sedangkan setiap artefak Analysis berikutnya ditautkan ke root artifact yang tenant-scoped. Satu sesi chat dapat memuat banyak eksplorasi sehingga baru menjadi family artikel ketika salah satu output dikomit.
  - Menuntaskan handoff Editor ke alur kerja: draft sumber tetap terkunci dan terlihat selama panel pipeline khusus berjalan, kemudian hasil yang sudah tersimpan durable menawarkan Review atau Publikasi sesuai finding current yang belum selesai. Selesaikan nanti kembali ke document home tanpa kehilangan hasil.
  - Membawa paket SEO hasil pipeline bersama artikel ready ke Publikasi dan memindahkan judul, slug, ringkasan, meta field, alt text sampul, tag, serta keputusan metadata ke panel kanan khusus alih-alih menumpuknya di atas artikel. Pada mobile, pemisahan yang sama tersedia melalui tab Artikel dan Paket SEO.
- **Tinjauan Pra-final Berbasis Keputusan**:
  ...
  - Membuat penerapan temuan field publikasi atomik: backend kini memvalidasi revisi aktif dan finding tersimpan di server, lalu menyimpan nilai metadata pilihan beserta resolusi `isApplied` dalam satu transaksi serializable. Guard readiness untuk penyimpanan metadata umum tetap dipertahankan.
...
- **Pengeditan Final Draft langsung di tempat**:
  ...
  - Perubahan body yang dibuat sistem melalui Terapkan, Terapkan Semua, Rewrite/Remove dengan EAI, dan penambahan sumber kini otomatis dilanjutkan ke Quality Check. Jika revisi tersebut benar-benar membuat paket publikasi lama berstatus `stale`, regenerasi SEO berjalan otomatis setelah body lulus; temuan yang masih tersisa tetap dikembalikan ke editor tanpa melemahkan guard ekspor.
  - Menambahkan invariant publikasi berbasis revisi tanpa migrasi schema: setiap Final Draft tersimpan kini membawa revision ID, hash SHA-256 body persis, identitas blok sidecar yang persisten, sinyal revisi, origin mutasi, dan `DraftChangeSet` terbatas. Quality Check, generasi SEO, serta resolusi editorial menolak revision mismatch; hasil publikasi berdurasi panjang memeriksa ulang revisi di dalam transaksi serializable sebelum disimpan agar respons AI lama tidak menimpa draft yang lebih baru.
  - Menambahkan identitas persisten yang diterbitkan backend untuk temuan Quality Gate, rule validator, klaim faktual/sensitif-sumber, blok target, serta sumber riset/terverifikasi kanonis. Rekonsiliasi History dan rerun kini mengutamakan identity tersebut dengan pencocokan teks lama hanya untuk record historis; field identity dari browser dibuang dan hanya dipulihkan dari temuan yang sudah disimpan server sebelum keputusan editor masuk ke ledger resolusi.
  - Menambahkan `ValidationScope` dependency-aware untuk pemeriksaan otomatis setelah mutasi. Revisi formatting-only memakai pemeriksaan deterministik tanpa panggilan AI, perubahan editorial terbatas memakai validasi ringan, sedangkan perubahan faktual/sensitif-sumber terbatas hanya memeriksa blok berubah, konteks tetangga, klaim/sumber terkait, serta anchor intro/kesimpulan global. Lineage hilang, topic shift, dan perubahan multi-blok luas tetap dieskalasi ke Quality Gate penuh. Hasil dijaga oleh revision ID/hash dan digunakan ulang secara idempoten untuk revisi persis yang sama.
  - Menambahkan background validation non-blocking setelah revisi manual Final Draft berhasil disimpan secara durable. Edit formatting/copy yang aman tidak menjadwalkan pekerjaan; revisi ringan memakai debounce 1,8 detik dan revisi high-risk 800 ms. Save baru serta aksi workspace yang berkonflik membatalkan/menggabungkan pekerjaan tertunda, editor tetap dapat mengetik selama pemeriksaan berjalan, dan UI melaporkan hasil atau keputusan tersisa tanpa menyuruh user rerun Quality Gate.
  - Mengganti invalidasi SEO seluruh paket dengan state dependensi persisten per-field. Field aman (`excerpt`, meta description, alt text sampul, dan tag) hanya diperbarui di background ketika isi lamanya bergantung pada draft yang berubah; judul, meta title, dan slug tidak pernah ditimpa otomatis dan tetap menjadi keputusan editor. Field yang belum selesai tetap membuat paket publikasi stale dan memblokir export.
  - Menyembunyikan mekanisme pipeline validasi dan SEO dari UX editorial normal. Final Draft kini menurunkan satu state berorientasi hasil—sedang diperiksa, selesai, perlu keputusan konten, atau perlu keputusan detail publikasi—tanpa menampilkan istilah stale/rerun. Pemeriksaan revisi manual dan penyegaran detail publikasi tetap tersedia secara kondisional sebagai alat pemulihan, sedangkan aksi Apply hanya menjelaskan perubahan editorial karena verifikasi menjadi tanggung jawab sistem.
  - Menegaskan konvergensi remediasi: penerapan saran bersifat sementara dan tidak lagi dapat menyembunyikan temuan sama yang masih ada pada validasi berikutnya. Hanya warning yang diterima editor atau sumber yang diverifikasi yang masuk ke ledger resolusi permanen; record `applied` dan cache validasi sebelum perbaikan diabaikan.
  - Mengganti penulisan ulang generatif untuk temuan source-fidelity dengan pilihan sumber, penghapusan/netralisasi deterministik, atau keputusan manual. Kandidat Targeted Fix wajib menghapus sinyal tanpa dukungan yang disebut, benar-benar mengubah target, tidak menambah sinyal baru, dan tidak membuat heading duplikat.
  - Menetapkan Analyze dan Refine sebagai satu-satunya mode analisis yang memotong kredit. Targeted Fix, validasi revisi, Quality Check, serta generasi/refresh SEO tetap tersedia ketika saldo nol dan tidak masuk jalur debit.
...
### Fixed
- **Kejelasan data Peta Konten**:
  - Menampilkan judul artikel, topik, tahap editorial, status registry, status ekspor CMS, kata kunci, sumber, pemilik, dan aksi sumber secara terpisah, bukan memakai ringkasan editorial review sebagai identitas artikel.
...
- **Tenant Content Memory dan Duplicate Guard**:
  ...
  - Menambahkan pemeriksaan overlap deterministik sebelum generasi AI. Kecocokan konten/judul yang identik dan benturan reservasi aktif selalu memblokir pekerjaan duplikat, sedangkan kemiripan topik yang lebih luas tetap advisory kecuali policy kalibrasi Phase 5 secara eksplisit mengaktifkan enforcement.
  ...
  - Menambahkan retrieval hybrid Phase 3 dengan kandidat full-text/trigram PostgreSQL yang tenant-filtered serta pencarian cosine pgvector 768 dimensi. Embedding Gemini dibuat secara asinkron melalui job BullMQ idempoten, diberi versi berdasarkan model/source hash, memiliki retry terbatas, dan tidak digunakan ketika stale.
  - Menambahkan klasifikasi overlap terstruktur Phase 4 untuk kandidat ambigu melalui PCA composer khusus serta override runtime AI per fungsi di Admin. Classifier melaporkan elemen yang sama/berbeda dan rekomendasi angle, tetapi tidak dapat menghasilkan `exact_duplicate` atau memblokir request.
  - Menginjeksikan metadata related-content yang aman untuk kolaborasi ke prompt Blueprint, Quick Draft, dan Draft from Notes agar generasi menghindari angle yang sudah tercakup tanpa menganggap artifact internal sebagai sumber fakta.
  - Menambahkan migrasi `20260729210000_add_content_memory_semantic_retrieval` untuk pgvector, pg_trgm, indeks HNSW/full-text, field lifecycle embedding, serta telemetry classifier/retrieval. Migrasi diterapkan ke database Neon production pada 29 Juli 2026; Prisma mengonfirmasi seluruh 28 migrasi sudah up to date.
...
- **Pesan Strategist Yatim Setelah Stream AI Gagal**:
  ...
  - Menambahkan fallback native Models API secara terbatas ketika Gemini Interactions mengakhiri stream Fast Chat tanpa teks jawaban. Request Search dicoba ulang melalui Google Search grounding native dan tidak lagi mengulang jalur tool-call Interactions yang gagal; bila grounding juga gagal, respons turun secara transparan menjadi jawaban tanpa Search dan kredit Search tidak dipotong. Diagnostik terminal tetap tersanitasi tanpa prompt maupun konten hasil generasi.

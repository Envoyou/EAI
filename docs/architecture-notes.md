...
State resolusi membedakan keputusan editorial tanpa mutasi (`isAccepted`) dari perubahan yang benar-benar diterapkan ke body (`isApplied`). Acceptance terakhir tanpa perubahan body dapat menghasilkan `ready`. Rewrite, remove/neutralize, auto-apply, atau penambahan source yang mengubah body mengikuti readiness backend lalu otomatis menjalankan Quality Gate atas body persis yang baru disimpan. Bila lulus dan backend menandai paket publikasi yang sudah ada benar-benar `stale`, SEO dibuat ulang dan diaudit sebagai paket lengkap; status `possibly_stale` tetap advisory dan tidak memicu pekerjaan AI tambahan. Edit manual `formatting_only`/`minor_copy_edit` mempertahankan state, `editorial_change` menampilkan advisory ringan, dan `high_risk_change` memblokir sampai Quality Gate lulus. Temuan yang tetap ada setelah validasi otomatis ditampilkan sebagai pekerjaan editorial nyata, bukan tombol pemeriksaan ulang.

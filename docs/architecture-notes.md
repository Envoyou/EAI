...
State resolusi membedakan keputusan editorial tanpa mutasi (`isAccepted`) dari perubahan yang benar-benar diterapkan ke body (`isApplied`). Acceptance terakhir tanpa perubahan body dapat menghasilkan `ready`. Rewrite, remove/neutralize, auto-apply, atau penambahan source yang mengubah body tetap mengikuti readiness backend dan mewajibkan Quality Gate ulang, tetapi relevansi SEO dinilai terpisah dari risiko faktual body. Edit manual `formatting_only`/`minor_copy_edit` mempertahankan state, `editorial_change` menampilkan advisory ringan, dan `high_risk_change` memblokir sampai Quality Gate lulus. UI menampilkan status pending recheck dan tidak menghitung item resolved sebagai remaining check.

...
*   **Outbound URL Safety**: Pengambilan URL melalui Strategist dan `/api/scrape` memakai `safe-url-fetch.ts`. Utility hanya menerima HTTP(S), menolak URL berkredensial dan alamat privat/lokal IPv4/IPv6, meresolusi hostname, serta memvalidasi ulang target setiap redirect untuk mengurangi risiko SSRF.

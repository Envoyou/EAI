...
Scope naik ke full Quality Gate bila change set tidak tersedia, topic shift terdeteksi, lebih dari empat blok berubah, lebih dari dua blok dihapus, atau cakupan klaim berubah besar. Hasil incremental menyimpan revision ID, body hash, level, status, scope, waktu, dan jumlah putaran pada `metadata._system.lastValidationResult`. Request otomatis berulang untuk revisi/hash yang sama memakai ulang hasil durable tersebut; guard transaksi serializable tetap menolak hasil yang terlambat. Batas konvergensi otomatis saat ini satu putaran validasi per revisi—temuan tersisa dikembalikan sebagai keputusan editorial, bukan memulai loop fix/validate tanpa batas.

State resolusi membedakan keputusan editorial tanpa mutasi (`isAccepted`) dari perubahan yang benar-benar diterapkan ke body (`isApplied`). Acceptance terakhir tanpa perubahan body dapat menghasilkan `ready`. Rewrite, remove/neutralize, auto-apply, atau penambahan source yang mengubah body mengikuti readiness backend lalu otomatis menjalankan revision-scoped validation atas body persis yang baru disimpan. Bila lulus dan backend menandai paket publikasi yang sudah ada benar-benar `stale`, SEO dibuat ulang dan diaudit sebagai paket lengkap; status `possibly_stale` tetap advisory dan tidak memicu pekerjaan AI tambahan. Edit manual `formatting_only`/`minor_copy_edit` mempertahankan state, `editorial_change` menampilkan advisory ringan, dan `high_risk_change` memblokir sampai pemeriksaan yang dipilih backend lulus. Temuan yang tetap ada setelah validasi otomatis ditampilkan sebagai pekerjaan editorial nyata, bukan tombol pemeriksaan ulang.

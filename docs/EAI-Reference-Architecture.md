...
- **`ContentReservation` & `DuplicateGuardEvent`**: Short-lived request claims prevent concurrent duplicate generation, while audit events retain deterministic/hybrid verdicts, classifier model/latency, controlled-enforcement decisions, overrides, and attributable human labels. Probable-duplicate blocking is gated by a global flag, deterministic organization cohort, minimum labeled sample count, and measured precision; exact/reservation blocks remain non-overridable.
- **Content Intelligence Snapshot**: A rebuildable request-time projection over at most 300 recent organization artifacts. It combines tenant-filtered vector pairs with metadata fallback to produce topic clusters, cannibalization risks, evidence-backed gaps, refresh/consolidation work, and internal-link opportunities without another model call. Lifecycle derivatives are treated as one family for pairwise recommendations.
- **Actionable Content Intelligence**: Phase 6.1 resolves collaboration-safe article presentation from publication/working-title/H1 metadata, exposes source navigation and CMS export state, and records tenant-scoped reposition, canonical, consolidation, archive, and false-positive decisions. Registry actions never mutate article bodies or external CMS pages automatically.

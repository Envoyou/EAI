...
4.  ~~**Shared Content Memory dan pencegahan topik duplikat**~~ (Phase 1–5 diimplementasikan pada Unreleased setelah v3.18.0):
    ...
    *   Phase 6 menambahkan Content Intelligence pada Content Map: topic cluster lintas bahasa ketika vector tersedia, cannibalization detection, content gap berbasis alternative-angle/lifecycle evidence, rekomendasi update, dan peluang internal link. Snapshot dihitung ulang dari maksimal 300 artefak terbaru, memiliki fallback metadata, dan tidak membutuhkan panggilan LLM tambahan atau migrasi database.
    *   Phase 6.1 membuat hasil tersebut dapat ditindaklanjuti: CSV khusus Content Map, deep-link ke draft sumber, perbandingan pasangan, repositioning, canonical relationship, konsolidasi manual yang eksplisit, archive registry, dan feedback bukan-kanibalisasi. Presentasi artikel memakai metadata publikasi/working title/H1 serta menolak prompt dan status editorial sebagai judul.
    *   Integrasi metrik performa CMS/Search Console, kalender editorial terjadwal, dan rekomendasi update berbasis traffic tetap menjadi pengembangan berikutnya.
...
5.  **Pengembangan & Optimasi EAI Chat & Draft**:
    ...
    *   ~~**Hardening Boundary Strategist**~~ (Diimplementasikan pada v3.12.2): Mengaktifkan validasi Zod runtime untuk Chat, Generate Plan, Quick Draft, Draft From Notes, dan lampiran; memindahkan counter throttling Strategist serta autosave ke Redis atomic lintas instance; menambahkan batas demo pada generator berbiaya; memverifikasi ownership sesi sebelum write; menerapkan egress policy dengan DNS pinning pada URL scraper; dan menghentikan logging/billing lanjutan ketika client memutus stream.

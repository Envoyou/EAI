...
Initial refinement is a transition out of Editor, not another permanent Editor
surface. The source snapshot stays visible and mutation-locked while
`EditorWorkflowPanel` temporarily replaces the optional writing tools with the
pipeline status. Partial refined output may accumulate in request state, but it
does not expose review controls. Only after the backend returns a durable
AnalysisLog identity does the panel show a handoff: unresolved current findings
open `/review`, while a current `ready` result opens `/publication`. Finish
later returns to the document home and the current queue projection retains the
saved result.

The same durable record owns the refined body, Quality Gate result, and any
generated publication package. Publication restores that package from History
and shows its title, slug, excerpt, meta title, meta description, cover-image
alt text, tags, and metadata decisions in a dedicated right panel before
offering metadata editing. The center canvas contains the article rather than a
stacked SEO card. Narrow screens preserve this separation through Article and
SEO Pack tabs. SEO generated by the publish-ready pipeline is therefore never
stranded in Editor.
...
The family boundary begins at a committed editorial output, not at the chat
session. Strategist Blueprint, Quick Draft, and Draft from Notes responses
receive a backend-issued durable `sourceRef`; Proceed to Editor carries it into
article metadata, and subsequent Analyze, Refine, Review, SEO, and export
records retain it. The backend additionally links derived Analysis artifacts
to the earliest matching root artifact inside the active organization. This
allows one Strategist conversation to explore several ideas without merging
